 ...
 */
void BSChooser::AddObject(const char *name, AutoProgram object)
{
 ...
 * Functionally, this is very close to {@link BSChooser#AddObject(const char *name, void *object) AddObject(...)}
 * except that it will use this as the default option if none other is explicitly selected.
 * @param name the name of the option
 * @param object the option
 */
void BSChooser::AddDefault(const char *name, AutoProgram object)
{
 ...
 */
AutoProgram BSChooser::GetSelected()
{
	std::string selected = m_table->GetString(kSelected, m_defaultChoice);
	return m_choices[selected];
}

void BSChooser::InitTable(ITable* subtable) {
	StringArray keys;
	m_table = subtable;
	if (m_table != NULL) {
		std::map<std::string, AutoProgram>::iterator iter;
...
ITable* BSChooser::GetTable() {
	return m_table;
}

std::string BSChooser::GetSmartDashboardType() {
